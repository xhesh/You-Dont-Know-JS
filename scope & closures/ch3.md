...
The `i = 3` assignment inside of `bar(..)` overwrites, unexpectedly, the `i` that was declared in `foo(..)` at the for-loop. In this case, it will result in an infinite loop, because `i` is set to a fixed value of `3` which is then incremented by for-loop with `i++`, `i` will be finally be set to `4` and that will forever remain `< 10`.
