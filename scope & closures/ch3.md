...
The `i = 3` assignment inside of `bar(..)` overwrites, unexpectedly, the `i` that was declared in `foo(..)` at the for-loop. In this case, it will result in an infinite loop, because `i` is first set to a fixed value of `3` and then incremented by the for-loop with the `i++` statement, thus resulting in `i` finally being set to `4` and that will forever remain `< 10`.
